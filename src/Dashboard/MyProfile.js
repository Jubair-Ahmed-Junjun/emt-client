...
const MyProfile = () => {
  ...
  const {
    ...
    fetch(
      `https://desolate-scrubland-35954.herokuapp.com/usersByEmail?email=${authUser?.email}`
    ).then((res) => res.json())
  );
  ...
  const handleForm = async (event) => {
    ...
    await fetch(
      `https://desolate-scrubland-35954.herokuapp.com/usersByEmail?email=${user.email}`,
      {
        method: "put",
        headers: {
